...
namespace Test
{
    class Program
    {
        ...
        {
            var pas = new Parser(new Scanner(""));
            var con = new DatabaseOne();
            //var tab = con.GetTable<Object>().First(o => o.Type == 5 && o.ID == 93000);
            var meta = con.GetTable<Object_Metadata>().FirstOrDefault(m => m.Object_ID == 90000 && m.Object_Type == 5);
            ...
            File.Delete(@"C:\Temp\examples\test.cs");
            using (var writer = new StreamWriter(File.OpenWrite(@"C:\Temp\examples\test.cs")))
            {
                writer.WriteLine(str);
            }
